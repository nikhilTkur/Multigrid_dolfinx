...
for i in range(coarsest_level, finest_level + 1):
    ...
    dof_dict = {}
    for i in range(0, V.dofmap.index_map.size_local * V.dofmap.index_map_bs):
        dof_dict[i] = tuple([round(i, 3) for i in V_dof_coord[i]])
        dof_dict[tuple([round(i, 3) for i in V_dof_coord[i]])] = i
    mesh_dof_list_dict[i] = (dof_dict, i)
...
def getJacobiMatrices(A):
    ...
    diag_A_inv = scp.sparse.csr_matrix(
        np.diag(1 / (A_mat.diagonal().reshape(A_mat.shape[0], 1))))
...
def Interpolation2D(vec_2h, level_coarse):
    ...
    for i in range(0, vec_h_dim):
        ...
        if coarse_dof != -1:                # Cannot directly inject the boundary values not present in coarse grid
            ...
            i_h = fine_coord[0] / element_size_fine
            j_h = fine_coord[1] / element_size_fine
